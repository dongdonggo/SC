...
function GetNav($pid=1)
{
	...
	$gourl = $row['linkurl'];
		if($row['picurl'] != '')
		 ...
		 $str .= '<li><a class="h-animate aclear active'.$row['id'].'" href="'.$gourl.'">'.$classname.'</a></li>';
		 $i++;
  }
	return $str;
}
function GetNavm($pid=1)
{
	global $dosql, $cfg_isreurl;
    $i=2;
	$str = '';
	...
	$gourl = $row['linkurl'];
		if($row['picurl'] != '')
			...
			$classname = $row['classname'];
		 $str .= '<a class="m_meanli m'.$row['id'].'" href="'.$gourl.'">'.$classname.'</a></span>';
		 $i++;
  }
	return $str;
}

function GetNav1($pid)
{
	global $dosql, $cfg_isreurl, $cid;
	...
	{
		$classname = $row['classname'];
		$str .='<li><a href="#" data-filter='.$row['id'].'> '.$classname.' </a></li>';
	}
